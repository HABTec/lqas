...
.service('CalendarService', function (SessionStorageService, $rootScope) {

    return {
        getSetting: function () {

            var dhis2CalendarFormat = {keyDateFormat: 'yyyy-MM-dd', keyCalendar: 'gregorian', momentFormat: 'YYYY-MM-DD'};
            var storedFormat = SessionStorageService.get('SYSTEM_SETTING');
...
.service('DateUtils', function ($filter, CalendarService) {
    ...
    return {
        ...
        getToday: function () {
            ...
            today = calendar.parseDate(calendarSetting.keyDateFormat.toLowerCase(), today);
            return calendar.formatDate(calendarSetting.keyDateFormat.toLowerCase(), today);
        },
        ...
        formatFromUserToApi: function (dateValue) {
            ...
            var calendarSetting = CalendarService.getSetting();
            var calendar = $.calendars.instance(calendarSetting.keyCalendar);
            dateValue = calendar.parseDate(calendarSetting.keyDateFormat.toLowerCase(), dateValue);
            dateValue = calendar.formatDate('yyyy-mm-dd', dateValue);
            return dateValue;
        },
        formatFromApiToUser: function (dateValue) {
            ...
            var calendarSetting = CalendarService.getSetting();
            var calendar = $.calendars.instance(calendarSetting.keyCalendar);
            dateValue = calendar.parseDate('yyyy-mm-dd', dateValue);
            return calendar.formatDate(calendarSetting.keyDateFormat.toLowerCase(), dateValue);
        },
        formatFromApiToUserIso: function(dateValue) {
            if (!dateValue) {
                return;
            }
            var calendarSetting = CalendarService.getSetting();
...
.service('DataEntryUtils', function($q, $translate, $filter, SessionStorageService, DialogService, OrgUnitService, OptionSetService){
    return {
        ...
        getPercent: function(op1, op2, withoutSymbol){
            op1 = dhis2.validation.isNumber(op1) ? parseInt(op1) : 0;
            op2 = dhis2.validation.isNumber(op2) ? parseInt(op2) : 0;
            if( op1 === 0){
                return 0;
            ...
            if( op2 === 0 ){
                return $translate.instant('missing_denominator');
            }
            var res = Number(parseFloat((op1 / op2)*100).toFixed(2));

            return withoutSymbol ? res : res + '%';
        },
                    ...
                    if( leftSide && rightSide ){
                        ...
                        if( op !== null ){
                            try{
                                ...
                                return $translate.instant('invalid_validation_expression') + ': ' + leftSide + ' ' + op + ' ' + rightSide;
                            }
                            var res = eval( leftSide + op + rightSide);
                            if( !res ){
                                vrs.push(vr);
        ...
        getIndicatorResult: function( ind, indicatorType, dataValues ){
            var factor = 1;
            ...
            if( ind.numExpression ){
                try {
                    ...
                    return $translate.instant('invalid_numerator_expression') + ': ' + ind.numExpression;
                }
            }
            ...
            if( ind.denExpression ){
                try{
                    ...
                    return $translate.instant('invalid_denominator_expression') + ': ' + ind.denExpression;
                }
            }

            if(indicatorType && indicatorType.factor){
            ...
            if (dataSets && dataSets.length) {
                for (var i = 0; i < dataSets.length; i++) {
                    if (dataSets[i].id === dataSetId ) {
                        return dataSets[i].access && dataSets[i].access.data && dataSets[i].access.data.write;
                    }
                }
            }
            return false;
        },
        userHasReadAccess: function (dataSetId) {
            var dataSets = SessionStorageService.get('ACCESSIBLE_DATASETS');
            dataSets = dataSets.dataSets;
            if (dataSets && dataSets.length) {
                for (var i = 0; i < dataSets.length; i++) {
                    if (dataSets[i].id === dataSetId ) {
                        return dataSets[i].access && dataSets[i].access.data && dataSets[i].access.data.read;
                    }
                }
            }
            return false;
